...
TECHLIB_DIR = $(CURDIR)/../../syn_libraries/45nm
...
help::
	...
	@printf "\033[31m\tsynthesis/nangate15\033[39m Invokes Synopsys Design Compiler to synthesize the CV32E40P using nangate15 as tech lib\n\n"
	...
	@printf "\033[31m\tcompile_sbst INPUT=[folder_app_name/main]\033[39m Cross-Compiles your program for the CV32E40P RISC-V ISA. (be aware that the application must be handle in a file named main.c) \n\t\033[0;33mExample: compile_sbst INPUT=programs/conv_FP/main\n\n"
	@printf "\033[1mLogic Simulation\033[0m\n"
	@printf "\033[31m\tquesta/lsim/rtl/shell_polimi_polito\033[39m Invokes QuestaSIM to perform an RT-level simulation of the program in shell mode (without compiling the code, since Polimi provides the .hex).\n"
	@printf "\033[31m\tquesta/lsim/rtl/gui_polimi_polito\033[39m Invokes QuestaSIM to perform an RT-level simulation of the sbst in gui mode for the Polimi-Polito environment.\n\n"
...
Old_help::
	...
	@printf "\033[31m\tquesta/lsim/compile-timing\033[39m Invokes Siemens QuestaSIM to compile CV32E40P RT-level and Gate-level descriptions for timing logic simulation\n"
	@printf "\033[31m\tquesta/lsim/rtl/shell\033[39m Invokes QuestaSIM to perform an RT-level simulation of the sbst in shell mode.\n"
	@printf "\033[31m\tquesta/lsim/rtl/shell_polimi_polito\033[39m Invokes QuestaSIM to perform an RT-level simulation of the sbst in shell mode (without compiling the code, since Polimi provides the hex).\n"
	@printf "\033[31m\tquesta/lsim/rtl/gui\033[39m Invokes QuestaSIM to perform an RT-level simulation of the sbst in gui mode.\n"
	@printf "\033[31m\tquesta/lsim/rtl/gui_polimi_polito\033[39m Invokes QuestaSIM to perform an RT-level simulation of the sbst in gui mode for the Polimi-Polito environment.\n"
	@printf "\033[31m\tquesta/lsim/gate/shell\033[39m Invokes QuestaSIM to perform a Gate-level functional simulation of the sbst in shell mode.\n"
	@printf "\033[31m\tquesta/lsim/gate/gui\033[39m Invokes QuestaSIM to perform a Gate-level functional simulation of the sbst in gui mode.\n"
...
#  questa/compile: .questa-compile-rtl
...
# This version only simulates without the compilation step, since Polimi provides only the .hex files
.questa-lsim_polimi_polito: 
...
questa/lsim/rtl/shell_polimi_polito: export RUN_GUI  = 0
questa/lsim/rtl/shell_polimi_polito: export RUN_GATE = 0
questa/lsim/rtl/shell_polimi_polito: VWORK           = work_rtl

questa/lsim/rtl/shell_polimi_polito: .questa-lsim_polimi_polito
...
questa/lsim/rtl/gui_polimi_polito: export RUN_GUI  = 1
questa/lsim/rtl/gui_polimi_polito: export RUN_GATE = 0
questa/lsim/rtl/gui_polimi_polito: VWORK           = work_rtl

questa/lsim/rtl/gui_polimi_polito: .questa-lsim-gui_polimi_polito
...
## Uses the input argument INPUT that is the path to a targeted program.
## (be sure that the targeted program is main.c, to allow the direct use of the booth file in memory).

## example: compile_sbst INPUT=programs/sbst/main

compile_sbst:
	
	$(MAKE) -C $(CURDIR)/sbst INITIAL_NAME=$(INPUT)
